...
To place a C<\> inside the string, you may escape it with another C<\>:
...
This is the "temporary empty list assignment" operator. It is equivalent to 

    $x = @{[m/PAT/g]};

It assigns the number of matches made by m/PAT/g to X. This means that X 
must be a valid lvalue (i.e. it must be something that can be assigned to).

It is commonly used as clearer idiomatic syntax for finding out how many
times a regex matches a string:

=head3 Example

    # count the total number of '=' and '~' characters in this string.
    my $_ = "B=====D~~";
    my $x =()= /=/g;    # $x is now 7

=head3 See also

L</X = Y>, L</X = (Y)>, L<http://goatse.cx>
...
combination of several operators or pieces of syntax. These Secret Operators
